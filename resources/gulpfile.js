var gulp = require('gulp'),
    gulpSequence = require('gulp-sequence'), //同步执行任务
    cleancss = require('gulp-clean-css'),//css压缩
    uglify = require('gulp-uglify'), //js压缩
    htmlMin = require('gulp-htmlmin'), //压缩html
    clean = require('gulp-clean'), //清空文件夹
    rev = require('juyuan-gulp-rev'), //更改版本名 md5后缀
    autoFx = require('gulp-autoprefixer'), //加浏览器前缀
    livereload = require('gulp-livereload');
    var revCollector = require('juyuan-gulp-rev-collector'); //gulp-rev 的插件，用于html模板更改引用路径
    var plumber = require('gulp-plumber'); // 可以阻止 gulp 插件发生错误导致进程退出并输出错误日志。
    var watch = require('gulp-watch');// 监听文件 修改 新增 删除
    var runSequence = require('run-sequence');  // 按顺序执行task
// 全局配置
var options = {
    dist_html_dir : '../application/views/templates', // 模版目录
    src_html_dir :'src/html/',
    src_html:'src/html/**/*.html',
    src_js : 'src/js/**/*.js',
    src_css: 'src/css/**/*.css',
    src_img : 'src/img/**/*.{png,jpg,gif,ico}',
};

//清空文件夹
gulp.task('clean', function() {
    return gulp.src('../public/assets/dist', {
        read: false
    })
    .pipe(clean({
		force:true
	}));
});
//压缩css/加浏览器前缀
gulp.task('css', function() {
    return gulp.src(options.src_css)
    .pipe(plumber())
    .pipe(autoFx({
		overrideBrowserslist: ['last 10 versions', 'Firefox >= 20', 'Opera >= 36', 'ie >= 8', 'Android >= 4.0'],
        cascade: true, //是否美化格式
        remove: false //是否删除不必要的前缀
    }))
    .pipe(cleancss({
        keepSpecialComments:"*"
    }))
    .pipe(rev())
    .pipe(gulp.dest('../public/assets/dist/css'))
    .pipe(livereload())
    .pipe(rev.manifest())
    .pipe(gulp.dest('../public/assets/dist/rev/css'));
});

//压缩js
gulp.task('js', function() {
    return gulp.src(options.src_js)
    .pipe(plumber())
    .pipe(uglify())
    .pipe(rev())
    .pipe(gulp.dest('../public/assets/dist/js'))
    .pipe(livereload())
    .pipe(rev.manifest())
    .pipe(gulp.dest('../public/assets/dist/rev/js'));
});
//压缩image
gulp.task('image', function() {
    return gulp.src(options.src_img)
    .pipe(rev())
    .pipe(gulp.dest('../public/assets/dist/img'))
    .pipe(rev.manifest())
    .pipe(gulp.dest('../public/assets/dist/rev/img'));
});
//改变css引用路径

//改变html引用路径
gulp.task('rev', function() {
    return gulp.src(['../public/assets/dist/rev/**/*.json', options.src_html_dir+'**/*.html'])
    .pipe(plumber())
    .pipe(revCollector({
        replaceReved: true,
        dirReplacements:{
          '/src/css':'/dist/css',
          '/src/js':'/dist/js',
          '/src/img':'/dist/img',
        }
    }))
    .pipe(htmlMin({
        collapseWhitespace: true,
        includeAutoGeneratedTags:false,
        removeComments:true,
        minifyJS: true,//压缩页面JS,
        minifyCSS: true,//压缩页面CSS
    }))
    .pipe(gulp.dest(options.dist_html_dir))
    .pipe(livereload());
});
// 替换css 中图片路径
gulp.task('revimg', function() {
    return gulp.src(['../public/assets/dist/rev/img/*.json', '../public/assets/dist/css/**/*.css'])
    .pipe(revCollector({
        replaceReved: true,
        dirReplacements:{
          '/src/img':'/dist/img'
        }
    }))
    .pipe(gulp.dest('../public/assets/dist/css'));
});
gulp.task('default', function(cb){
    gulpSequence('clean', 'css', 'js', 'image', 'rev','revimg')(cb);
});
gulp.task('dev', function(cb){
    gulpSequence( 'css', 'js','image', 'rev','revimg')(cb);
});

gulp.task('watch',function(){
    livereload.listen({
        start:true
    });

    w(options.src_html,'rev');
    w(options.src_js,'js');
    w(options.src_css,'css');
    w(options.src_img,'image');

    function w(path,task){
        watch(path,function(){
            if (task === 'rev') {
                runSequence(task);
            }else{
                runSequence(task,'rev');
            }
        });
    }
});





